...
## 11.8 Место для оптимизации

| Область | Тип | Сложность | Описание |
|---------|-----|-----------|----------|
| GPU loss computation | 🚀 Perf | 🟡 Средняя | Вычисление loss на GPU, избежать CPU sync |
| Focal loss | 🔧 Feature | 🟢 Низкая | Для imbalanced classification |
| Label smoothing | 🔧 Feature | 🟢 Низкая | Регуляризация через soft labels |
| Custom loss API | 🔧 Feature | 🟡 Средняя | Trait для пользовательских loss functions |
| Automatic differentiation | 🔧 Feature | 🔴 Высокая | Автоградиент для произвольных loss |
