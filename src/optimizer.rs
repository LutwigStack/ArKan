...
impl AdamConfig {
    ...
    pub fn with_decay(lr: f32, weight_decay: f32) -> Self {
        Self { lr, weight_decay, ..Default::default() }
    }
}
...
impl Adam {
    ...
    pub fn new(network: &KanNetwork, config: AdamConfig) -> Self {
        let layer_states = network.layers
            .iter()
            .map(LayerAdamState::new)
            .collect();
        
        Self { config, layer_states }
    }
...
impl StepLR {
    pub fn new(initial_lr: f32, step_size: usize, gamma: f32) -> Self {
        Self { initial_lr, gamma, step_size }
    }
}
