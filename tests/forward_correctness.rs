...
fn test_forward_deterministic() {
    ...
    for (i, (a, b)) in output1.iter().zip(output2.iter()).enumerate() {
        assert_eq!(
            a, b,
            "Non-deterministic output at [{}]: {} vs {}",
            i, a, b
        );
    }
    ...
    for b in 0..batch_size {
        ...
        for i in 0..config.output_dim {
            ...
            assert!(
                ...
                b, i, single_output[i], batch_output[out_start + i], diff
            );
        }
    }
    
    println!("✓ forward_single vs forward_batch parity: max_diff = {:.2e}", max_diff);
}
...
fn test_forward_batch_vs_parallel_parity() {
    ...
    println!("✓ forward_batch vs forward_batch_parallel parity: max_diff = {:.2e}", max_diff);
}
...
fn test_simd_coverage_matrix() {
    let in_dims = [1, 3, 4, 7, 8, 9, 15, 16, 17, 31, 32, 33, 63, 64, 65, 127, 128];
    let simd_widths = [4, 8];
    ...
    println!("✓ SIMD coverage matrix: {}/{} combinations passed", passed, total);
}
