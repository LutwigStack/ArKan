...
fn test_async_download_correctness() {
    ...
    loop {
        ...
        if let Ok(result) = rx.try_recv() {
            ...
            println!("✅ Async download returned correct data ({} elements)", data.len());
            return;
        }
...
fn test_async_download_vs_sync_parity() {
    ...
    // Sync download
    let sync_result = tensor.download(&backend.device, &backend.queue)
...
fn test_async_download_callback_called_once() {
    ...
    assert_eq!(count, 1, "Callback should be called exactly once, got {}", count);
...
fn test_large_tensor_10mb() {
    ...
    for i in (0..data.len()).step_by(10000) {
        assert_eq!(
            data[i].to_bits(), downloaded[i].to_bits(),
            "Mismatch at index {}", i
        );
    }

    println!("✅ 10MB tensor: upload={:?}, download={:?}", upload_time, download_time);
}
...
fn test_large_tensor_100mb() {
    ...
    let data: Vec<f32> = (0..num_elements).map(|i| (i as f32 * 0.0001).cos()).collect();
    ...
    println!("✅ 100MB tensor: upload={:?} ({:.1} MB/s), download={:?} ({:.1} MB/s)",
             upload_time, throughput_upload,
             download_time, throughput_download);
}
...
fn test_large_tensor_near_max_buffer() {
    ...
    println!("✅ 200MB tensor: upload={:?}, download={:?}", upload_time, download_time);
}
...
fn test_max_buffer_size_documented() {
    ...
    println!("ℹ️  Device max_buffer_size = {} bytes ({} MB)",
...
fn test_large_tensor_500mb() {
    ...
    if max_buffer_size < required_size {
        println!("⚠️  SKIPPED: wgpu max_buffer_size ({} MB) < required ({} MB)",
                 max_buffer_size / 1024 / 1024, required_size / 1024 / 1024);
    ...
    println!("✅ 500MB tensor: upload={:?}, download={:?}", upload_time, download_time);
}
...
fn test_large_tensor_2gb() {
    ...
    println!("Testing 2GB tensor ({} MB, {} elements)...", 
             required_bytes / 1024 / 1024, num_elements);
    ...
    let downloaded = tensor.download(&backend.device, &backend.queue)
        .expect("Download failed");
    let download_time = start.elapsed();
    println!("  Download: {:?} ({:.1} GB/s)", 
             download_time,
             (required_bytes as f64 / 1e9) / download_time.as_secs_f64());

    // Verify data integrity (sample check)
    assert_eq!(downloaded.len(), data.len());
    let samples = [0, 1000, 1_000_000, 100_000_000, 500_000_000, num_elements - 1];
    for &i in &samples {
        assert_eq!(
            data[i].to_bits(), downloaded[i].to_bits(),
            "Mismatch at index {}", i
    ...
    println!("✅ 2GB tensor: upload={:?}, download={:?}", upload_time, download_time);
}
...
fn test_large_tensor_3gb() {
    ...
    println!("ℹ️  Backend max_vram_alloc = {} GB", backend.max_vram_alloc() / 1024 / 1024 / 1024);

    // 3GB = 3 * 1024 * 1024 * 1024 bytes = 768M f32 elements
    let num_elements: usize = 3 * 1024 * 1024 * 1024 / 4;
    let required_bytes = (num_elements * 4) as u64;
    
    println!("Testing 3GB tensor ({} MB, {} elements)...", 
             required_bytes / 1024 / 1024, num_elements);
    ...
    // Verify data integrity (sample check)
    assert_eq!(downloaded.len(), data.len());
    let samples = [0, 1000, 1_000_000, 500_000_000, 700_000_000, num_elements - 1];
    for &i in &samples {
        assert_eq!(
            data[i].to_bits(), downloaded[i].to_bits(),
            "Mismatch at index {}", i
    ...
    println!("✅ 3GB tensor: upload={:?}, download={:?}", upload_time, download_time);
}
...
fn test_vram_limit_percent() {
    ...
    println!("ℹ️  Device max_buffer_size = {} MB", device_max / 1024 / 1024);
    println!("ℹ️  30% limit = {} MB", backend.max_vram_alloc() / 1024 / 1024);
    ...
    assert_eq!(VramLimit::Gigabytes(2).resolve(1000), 2 * 1024 * 1024 * 1024);
...
fn test_large_tensor_with_percent_limit() {
    ...
    if required_bytes > max_alloc {
        println!("⚠️  SKIPPED: 1GB > 30% limit ({} MB)", max_alloc / 1024 / 1024);
        return;
    }
    ...
    assert_eq!(downloaded[num_elements - 1].to_bits(), data[num_elements - 1].to_bits());
    
    println!("✅ 1GB tensor with 30% limit: upload={:?}, download={:?}", upload_time, download_time);
}
...
fn test_alignment_odd_element_counts() {
    ...
    for &size in &test_sizes {
        ...
        assert_eq!(downloaded.len(), data.len(), "Size {} length mismatch", size);
        for (i, (&e, &g)) in data.iter().zip(downloaded.iter()).enumerate() {
            assert_eq!(
                e.to_bits(), g.to_bits(),
                "Size {} mismatch at index {}", size, i
            );
        }
    }
...
fn test_alignment_2d_shapes() {
    ...
    for (rows, cols) in test_shapes {
        ...
        let tensor = GpuTensor::upload(
            &backend.device, &backend.queue, &data, vec![rows, cols]
        ...
        for (i, (&e, &g)) in data.iter().zip(downloaded.iter()).enumerate() {
            assert_eq!(e.to_bits(), g.to_bits(), 
                       "Shape [{}, {}] mismatch at index {}", rows, cols, i);
        }
    }
...
fn test_alignment_f32_natural() {
    ...
    for &size in &sizes {
        ...
        assert!(
            ...
            size, tensor.capacity_bytes, expected_bytes
        );
...
fn test_stress_many_small_tensors() {
    ...
    for i in 0..num_tensors {
        let data: Vec<f32> = (0..tensor_size).map(|j| (i * tensor_size + j) as f32).collect();
    ...
    for &i in &samples {
        let expected: Vec<f32> = (0..tensor_size).map(|j| (i * tensor_size + j) as f32).collect();
        let downloaded = tensors[i].download(&backend.device, &backend.queue)
    ...
    println!("✅ {} small tensors allocated/verified in {:?}", num_tensors, alloc_time);
}
...
fn test_stress_rapid_upload_download() {
    ...
    for i in 0..iterations {
        ...
        // Spot check
        assert_eq!(downloaded[0].to_bits(), data[0].to_bits());
        assert_eq!(downloaded[tensor_size/2].to_bits(), data[tensor_size/2].to_bits());
        assert_eq!(downloaded[tensor_size-1].to_bits(), data[tensor_size-1].to_bits());
    }
    
    let total_time = start.elapsed();
    let per_iteration = total_time / iterations as u32;

    println!("✅ {} upload/download cycles in {:?} ({:?}/iter)", 
...
fn test_stress_mixed_sync_async() {
    ...
    println!("✅ {} mixed sync/async operations in {:?}", num_ops, total_time);
}
...
fn test_single_element_tensor() {
    ...
    let tensor = GpuTensor::upload(&backend.device, &backend.queue, &data, vec![1])
        .expect("Upload failed");
...
fn test_special_float_values() {
    ...
    for (i, (&expected, &got)) in data.iter().zip(downloaded.iter()).enumerate() {
        assert_eq!(
            ...
            i, expected, expected.to_bits(), got, got.to_bits()
        );
    }
...
fn test_nan_inf_preservation() {
    ...
    let tensor = GpuTensor::upload(&backend.device, &backend.queue, &data, vec![4])
        .expect("Upload failed");
    ...
    assert!(downloaded[1].is_infinite() && downloaded[1].is_sign_positive(), "Inf not preserved");
    assert!(downloaded[2].is_infinite() && downloaded[2].is_sign_negative(), "-Inf not preserved");
