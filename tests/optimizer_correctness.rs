...
fn test_adam_formula_numerical() {
    ...
    for (i, (&w, &g)) in initial_weights
        .iter()
        .zip(weight_grads[0].iter())
        .enumerate()
    {
        let mut m = 0.0f32;
        let mut v = 0.0f32;
...
fn test_adam_bias_correction_factors() {
    ...
    for t in 1..=10 {
        ...
        optimizer
            .step(&mut network, &grad_weights, &grad_bias, None)
            .unwrap();
    ...
    for t in 1..=5 {
        ...
        println!(
            "t={}: bc1={:.6}, bc2={:.6}, factor={:.6}",
            t, bc1, bc2, correction_factor
        );
    }
...
fn test_adam_weight_decay_formula() {
    ...
    // Step with decay
    optimizer_decay
        .step(&mut network, &zero_grad_weights, &zero_grad_bias, None)
        .unwrap();
    ...
    println!(
        "Weight with decay: {} (expected: {})",
        weight_with_decay, expected_with_decay
    );
...
fn test_adam_custom_betas() {
    ...
    for (name, config) in &configs {
        ...
        optimizer
            .step(&mut network, &grad_weights_neg, &grad_bias, None)
            .unwrap();
        let weight_after_neg = network.layers[0].weights[0];

        println!(
            "{}: {} -> {} -> {}",
            name, initial_weight, weight_after_pos, weight_after_neg
        );
...
fn test_adam_momentum_accumulation() {
    ...
    for t in 1..=5 {
        ...
        println!(
            "t={}: m={:.6} (expected {:.6}), v={:.6} (expected {:.6})",
...
mod gpu_tests {
    ...
    fn test_gpu_adam_vs_cpu_adam_single_step() {
        ...
        println!(
            "Hybrid loss: {:.6}, Native loss: {:.6}, diff: {:.2e}",
        ...
        gpu_native
            .sync_weights_to_cpu(&mut cpu_check)
            .expect("Sync failed");

        for (layer_idx, (hybrid_layer, native_layer)) in cpu_for_hybrid
            .layers
            .iter()
            .zip(cpu_check.layers.iter())
            .enumerate()
        {
    ...
    fn test_gpu_adam_momentum_parity() {
        ...
        for step in 0..num_steps {
            ...
            let loss_diff = (hybrid_loss - native_loss).abs();
            println!(
                "Step {}: Hybrid loss={:.6}, Native loss={:.6}, diff={:.2e}",
